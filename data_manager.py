...
def load_chart_data(stock_code):
    ...
    # sql = " select date, open, high, low, close, volume from coin.MAIMAIDATA where NAME='%s' order by NO desc " % (stock_code)
    # sql = " select date, open, high, low, close, volume from coin.STUDYDATA where NAME='%s' order by NO asc  " % (stock_code)
    sql = " select date, open, high, low, close, volume from coin.MAIMAIDATA where (NAME='%s' and open!='0') order by NO asc  " % (stock_code)

    chart_data = pd.read_sql(sql, connmysql)
    # chart_data = chart_data.dropna(how='any')
    # chart_data = chart_data.fillna(chart_data.mean())
...
def build_training_data(prep_data):
    ...
    # print(training_data['open'][1:].values, training_data['close'][:-1].values, training_data['close'][:-1].values);
