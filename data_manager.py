...
def load_chart_data(stock_code):
    ...
    sql = " select date, open, high, low, close, volume from coin.MAIMAIDATA where NAME='%s' order by NO asc  " % (stock_code)
...
def build_training_data(prep_data):
    ...
    print("open : ", training_data.loc[1:, 'open_lastclose_ratio']);
